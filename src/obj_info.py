...
class ObjectInfo:
    ...
    def add_object_grasp(self, obj_id, plan: Pose, quality):
        ...
        if obj_type in self.grasp_type_count:
            if len(self.obj_dict[obj_id]["grasp_plans"]) == 1:
                self.grasp_type_count[obj_type] += 1
        else:
            self.grasp_type_count[obj_type] = 1
        rospy.logdebug(f"物体类型 {obj_type} 现在有 {self.grasp_type_count[obj_type]} 个可抓物体")

    def merge_object_pointcloud(self, point_cloud, obj_type):
        rospy.logdebug(f"融合类型为 {obj_type} 的物体点云")
        ...
        if max_match_id != -1:
            ...
            rospy.logdebug(
                f"融合到匹配的物体:{max_match_id} 匹配点数:{max_num_matches} 融合后点数:{len(self.obj_dict[max_match_id]['point_cloud'])}"
            )
        else:
            obj_id = 1
            while obj_id in self.obj_dict:
                obj_id += 1
            rospy.logdebug(f"融合未匹配，新建物体: {obj_id}  点数: {len(point_cloud)}")
            self.new_object(obj_id, obj_type)
            self.add_object_pointcloud(obj_id, point_cloud)

    def get_best_grasp_plan_and_remove(
        self, obj_type
    ) -> tuple[Pose | None, list[npt.NDArray] | None, npt.NDArray | None]:
        rospy.logdebug(f"获取最佳抓取并移除一个类型为 {obj_type} 的物体")
        best_quality = 0
        best_plan = None
        best_obj_id = None
        best_obj_type = None
        ...
        rospy.logdebug(f"最佳目标ID: {best_obj_id}")
        if best_obj_id is None:
            return None, None, None
        plans = self.obj_dict[best_obj_id]["grasp_plans"]
        del self.obj_dict[best_obj_id]
        self.grasp_type_count[best_obj_type] -= 1
        return best_plan, plans, best_obj_mask

    def find_object_by_position(self, position):
        rospy.logdebug(f"根据位置查找物体: {position}")
        nn = NearestNeighbors(n_neighbors=1)
        max_num_matches = 0
        max_match_id = -1
        for obj_id, obj_info in self.obj_dict.items():
            ...
            if num_matches > max_num_matches:
                ...
                max_match_id = obj_id
        if max_match_id != -1:
            rospy.logdebug(f"找到物体 {max_match_id} 匹配点数: {max_num_matches}")
            return max_match_id
        return None
